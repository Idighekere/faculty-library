...
const getBooksByCourse = catchAsync(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
...
const addBook = catchAsync(async (req: Request, res, next) => {
    ...
    if (!course) {
        return next(new ErrorResponse("Course not found. The course must be added by an admin before", 404))
    }

    const book = await Book.create({ title, driveUrl, course: course._id, category, uploadedBy: req?.user._id })
...
const updateBook = catchAsync(async (req, res, next) => {
    ...
    if(req.user._id!==book.uploadedBy){
        return next(new ErrorResponse("You are not authorized to update this book", 401))
    }

    const updatedBook = await Book.findByIdAndUpdate(bookId, { title, driveUrl, course, category }, { new: true })
...
export { getBooksByCourse, addBook, getBooksByUser, updateBook }
